...
temp_button_color = ft.Colors.AMBER
temp_button_text_color = ft.Colors.BLACK
...
error_text_color = ft.Colors.BLACK
...
def main(page: ft.Page):
    ...
    def call_weather_api(search_bar):
        gui_values["search_bar"].value = search_bar.content.value
        get_weather(gui_values)


    def temp_change(temp_button, temp_list):
        temp_unit = temp_button.content.text
        new_unit = '\u00B0C' if temp_unit == '\u00B0F' else '\u00B0F'
        temp_button.content.text = new_unit
        gui_values["temp_unit"].text = new_unit

        for temp in temp_list:
            if f'--{temp_unit}' in temp.content.value:
                if temp.content.value == f'Feels like --{temp_unit}':
                    temp.content.value = f'Feels like --{new_unit}'
                else:
                    temp.content.value = f'--{new_unit}'
                temp.update()
    ...
    search_button = ft.Container(
        content = ft.IconButton(icon = ft.Icons.SEARCH, icon_color = search_icon_color, bgcolor = transparent_black,
                                on_click = lambda e: call_weather_api(search_bar)),
        padding = ft.padding.only(top = 10),
    )

    temp_button = ft.Container(
        content = ft.ElevatedButton(text = "\u00B0F", color = temp_button_text_color, bgcolor = temp_button_color,
                                    on_click = lambda e: temp_change(temp_button, [current_temp_container, max_temp_container, min_temp_container, feels_like_container])),
        padding = ft.padding.only(top = 10)
    )
    ...
    max_min_stack = ft.Container(
        content = ft.Stack(controls = [max_min_container,max_temp_container,min_temp_container,]),
        padding = ft.padding.only(left = width/3.3)
    )
    ...
    def sun_time_container(color_gradient, sun_time, text_color, time_value):
        ...
        template_stack = ft.Stack([template2, solar_transition, time,])
    ...
    def weather_info_container(icon_value, text_value, info_value):
        ...
        weather_info_icon = ft.Container(ft.Icon(icon_value, size = 24), padding = ft.padding.only(left = 10, top = 10))
        weather_info_title = ft.Container(ft.Text(value = text_value, size = 15, weight = 'w300'), padding = ft.padding.only(left = -5, top = 10))
        weather_info_value = ft.Container(content = ft.Text(value = info_value, text_align = ft.TextAlign.LEFT, weight = 'w600', size=20),
                                          padding = ft.padding.only(top = height/30, left = 25, bottom = 15),alignment = ft.alignment.center_left)
        
        info_stack = ft.Container(
            content = ft.Stack([info_box,ft.Row([weather_info_icon, weather_info_title]),weather_info_value])
        )
        return(info_stack)
    ...
    error_container = ft.Container( content = ft.Text(value = f'', color = error_text_color, size = 15, weight = 'w600'),
                                    padding = ft.padding.only(top = 20)
                        )

    gui_values = {
        "temp_unit": temp_button.content,
        ...
        content = ft.Column(
            controls = [
               ft.Row(
                ...
                ft.Row(
                    [search_bar, search_button,temp_button], alignment = ft.MainAxisAlignment.CENTER
                ),
                section_divider,
